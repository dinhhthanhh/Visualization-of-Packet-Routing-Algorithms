    ...
    void showGraph(Pane pane){
        ...
        tempPane.setOnMouseClicked(mouseEvent -> {
            if (addNode.isSelected()) {
                td.getEditor().clear(); // Xóa nội dung cũ trong hộp thoại trước khi hiển thị
                td.showAndWait().ifPresent(input -> {
                    // Chỉ thêm node nếu người dùng đã nhập tên
                    if (!input.trim().isEmpty()) {
                        tempPane.getChildren().clear();
                        graph.addNode(mouseEvent.getX() / slider.getValue(), mouseEvent.getY() / slider.getValue(), input);
